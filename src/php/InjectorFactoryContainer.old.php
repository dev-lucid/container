...
{
    /**
     * Array of configured constructors.
     * @var array
     */
        ...
        if ($has === false) {
            foreach ($this->constructors as $constructorId=>$constructor) {
                if (strpos($id, $constructorId) === 0) {
                    $this->copyConstructor($id, $constructor['className']. substr($id, strlen($constructorId)), $constructor);
    ...
    protected function copyConstructor(string $newId, string $newClassName, array $constructor)
    {
        #echo("<hr />\nCopying constructor $newClassName:\n");
        #print_r($constructor);
        $this->constructors[$newId] = [
    ...
    public function findContainerForDelegateParameter(string $name, string $type, bool $isScalar, string $scalarContainerId='')
    {
         ...
         * Match by class prefix in constructors
        */
        ...
        # at this point, the only options left are objects/constructors
        if ($isScalar === false) {
            ...
            foreach($this->constructors as $classPrefix => $constructor) {
                if (strpos($type, $classPrefix) === 0) {
                    $this->copyConstructor($type, $type, $constructor);
                    $this->addParameter($type, 'fixed', 'name', $name);
                    return $this;
                }
            }
            ...
            /*
            foreach ($this->constructors as $constructorId=>$constructor) {
                if (strpos($type, $constructorId) === 0) {
                    $this->constructors[$type] = [
                        'className' =>   $constructor['className']. substr($type, strlen($constructorId)),
                        'closure'=>      &$constructor['closure'],
                        'isSingleton' => &$constructor['isSingleton'],
                        'parameters' =>  &$constructor['parameters'],
                        'instantiationClosures' => &$constructor['instantiationClosures'],
                    ];
                    #$this->addParameter($type, 'fixed', 'name', substr($type, strlen($constructorId)));
                    return $this;
                }
            }
            */
    ...
    public function buildInjectableParameters(array $reflectionParameters, array $configuredParameters) : array
    {
        ...
        foreach ($reflectionParameters as $reflectionParameter) {
            ...
            if (isset($configuredParameters[$name]) === true) {
                ...
                if ($container !== false) {
                    ...
                    #echo("Could not find a container for parameter, name=$name, type=$type, isScalar=$isScalar\n");
                }
            }
        ...
        #print_r($reflectionParameters);
        #print_r($parameters);
        #echo('<hr />');
        return $parameters;
    }

    public function construct(string $id, string $type = '', array $constructor=null)
    {
        if (is_null($constructor) === true) {
            $constructor = $this->findConstructor($id, $type);
        }
